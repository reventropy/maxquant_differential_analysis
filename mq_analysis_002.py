...
class DesignParser:
	...
	def parse(self,path):
		...
		for header in root.iter("PastedHeader"):
			self.run.pastedheaders.append(header.attrib.get('name'))
		...
		for i in root.iter("impute"):
			self.run.imputation = ImputationSchema(i.attrib.get('itype'),i.attrib.get('headers'))
...
class ImputationSchema:
	def __init__(self,itype,headers):
		self.itype = itype
...
def apply_filter(ftype,f_threshold,headers,df): ## all filters are inclusive
	...
	if ftype == 'drop_nans':
		cnt=0
		idx2drop = []
		for val in df[headers].get_values():
			if isFloat(val):
				idx2drop.append(cnt)
			cnt+=1
		df = df.drop(df.index[idx2drop])
	return df
...
def generate_series(analysis_type,df):
	...
	[series_df.insert(series_df.shape[1],header,df[header].values) for header in design.run.pastedheaders]
...
class Imputation:
	...
	@staticmethod
	def medianFillImputation(self,df,headers):
	...
	@staticmethod
	def gbr_impute(self,df,headers):
		...
		for col in range(data.shape[1]):
			...
			for row in data:
				if np.isnan(row[col]):
					col_indices = self.return_non_missing_cols(row)	
...
class StatisticalTest:
	...
	@staticmethod
	def single_ttest(self,df,C1):
		G1vals = df[C1].get_values()
		popmean = np.mean(G1vals[~np.isnan(G1vals)])
		pvals=[]
		stat=[]
		for ridx in range(G1vals.shape[0]):
			cvals = G1vals[ridx,:]
			vals = ttest_1samp(cvals[~np.isnan(cvals)],popmean)
			pvals.append(vals[1])
			stat.append(vals[0])
		df.insert(df.shape[1],'oneSample_ttest_pvals',pvals)
		df.insert(df.shape[1],'oneSample_ttest_stat',stat)
		return df
...
class DifferentialAnalysis:
	...
	def filter_absent_conditions(self,df,n_conds,ctype):
		...
		for c in design.run.conditions:
			...
			for idx in range(missing_sum.shape[0]):
				if missing_sum[idx]==len(groupheaders):
					zmap[idx]+=1
		df.insert(df.shape[1],'sum_missing_conditions',zmap)
		df = apply_filter('lowpass',n_conds,['sum_missing_conditions'],df)
		df.to_csv("preprocessed.txt",sep="\t")
		return df		
	...
	def runStatTests(self,df):
		...
		for test in design.run.tests:
			if design.run.runtype=="Silac":
				if test.ttype!='single_ttest':
					log("Warning: Reverting to One Sample TTEST for SILAC ratio data!",1)
				func = getattr(Test,'single_ttest')
				df = func(Test, df, design.run.c2RNames[test.c2])
				return df
	...
	def impute(self,df):
		'''
		By default, imputation is done on all raw data columns one condition at a time.  This should ensure that there are no
		missing values downstream (unless the run is N=1).
		'''
		I = Imputation
		if design.run.imputation is None:
			return df
		for c in design.run.conditions:
				cols = design.run.c2RNames[c]
				func = getattr(I,design.run.imputation.itype)
				df = func(I,df,cols)
		return df
...
class ModificationAnalysis(DifferentialAnalysis):
	def __init__(self,current_analysis):
		...
		series_df.to_csv(self.get_outpath()+"phosphosite_diffexp.txt",sep="\t",index=False)
...
if __name__ == "__main__":
	...
	design = DesignParser(mqrun_path+"EGF_proteins.xml")
